...
import { validateItems, sanitizeString, generateRequestId } from '@/lib/validation';

export async function POST(request) {
    const requestId = generateRequestId();
    const rl = checkRateLimit(request, 'recommend-foods');
    ...
    try {
        ...
        const storeName = sanitizeString(body?.storeName ?? '', 100);

        const itemsList = items.map(i => `- ${i.name} (${i.category}, $${i.price})`).join('\n');
...
IMPORTANT: Return ONLY valid JSON. No markdown, no explanations.`;
        ...
        try {
            ...
            console.warn(`[${requestId}] AI recommendations failed:`, aiError.message);
            const fallbackRecs = items.map(item => ({
                original_item: item.name,
                same_store_alternative: {
                    name: `Store Brand ${item.name}`,
                    reason: 'AI unavailable — try again later',
                    price_impact: 'Usually cheaper',
                },
                best_health_alternative: {
                    name: `Organic ${item.name}`,
                    reason: 'Usually fewer pesticides and healthier processing',
                    price_impact: 'Varies',
                },
            }));
            ...
            return NextResponse.json({
                ...
                provider: 'local',
                warning: 'AI unavailable; showing generic recommendations',
                requestId,
            }, { headers: rateLimitHeaders(rl) });
        }
    } catch (error) {
        ...
        return NextResponse.json(
            { error: 'Recommendation failed', requestId },
